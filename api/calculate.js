...
export default async function handler(req, res) {
 ...
 const tier4Scoring = {
   ...
   jointPain: { // Inflammation systémique
     'Jamais': 100,
     'Après effort intense uniquement': 75,
     'Le matin au réveil': 50,
     'Régulièrement dans la journée': 25,
     'Douleurs chroniques permanentes': 0
   },
   skin: { // Marqueur de vieillissement
     'Éclatante et ferme': 100,
     'Correcte pour mon âge': 75,
     'Terne et fatiguée': 50,
     'Rides marquées': 25,
     'Très vieillie prématurément': 0
   },
   last100: { // État général
     'Cette semaine': 100,
     'Ce mois-ci': 75,
     'Cette année': 50,
     'L\'année dernière': 25,
   ...
   crash: { // Énergie quotidienne
     'Jamais': 100,
     'Après 17h': 75,
     'Vers 14h-15h': 50,
     'Juste après le déjeuner': 25,
     'Dès le matin': 0
   },
   projection: { // Conscience santé
     'En meilleure forme (j\'optimise déjà)': 100,
     'Stable (stagnation)': 60,
     'Diminué(e) de 20%': 40,
     'Très diminué(e) de 40%': 20,
     'J\'ai peur d\'y penser': 0
   },
   barriers: { // Motivation
     'Trop d\'infos contradictoires': 60,
     'Manque de temps': 50,
     'J\'ai essayé sans résultats': 40,
     'Trop cher': 30,
     'Je ne savais pas comment': 70
   }
 };
 ...
 // BONUS pour les femmes selon leur cycle
 if (answers.gender === 'femme' && answers.femaleSpecific) {
   const femaleBonus = {
     'Cycle régulier parfait': 50,
     'Cycle irrégulier': 0,
     'Grossesse': 30,
     'Post-partum': -20,
     'Péri-ménopause': -10,
   ...
   totalScore += femaleBonus[answers.femaleSpecific] || 0;
 }

 // BONUS pour les hommes selon libido
 if (answers.gender === 'homme' && answers.libido) {
   const libidoBonus = {
     'Au top comme à 20 ans': 50,
     'Correcte': 25,
     'En baisse notable': 0,
     'Très diminuée': -25,
     'Problématique': -50
   };
   totalScore += libidoBonus[answers.libido] || 0;
 }

 // Score final sur 100
 const finalScore = Math.max(0, Math.min(100, Math.round((totalScore / maxPossible) * 100)));
...
try {
  ...
  // Préparer les données avec les BONS NOMS pour Google Sheets
  const dataForSheet = {
    ...
    score: finalScore,
    age_exact: answers.age || '',
    gender: answers.gender || '',
    weight: answers.weight || '',
    height: answers.height || '',
    bmi: answers.imc || '',
    objectives: Array.isArray(answers.goals) ? answers.goals.join(', ') : (answers.goals || ''),
    energy_vs_3years: answers.energy3y || '',
    last_100_percent: answers.last100 || '',
    change_blocker: answers.barriers || '',
    stairs_test: answers.stairs || '',
    sitting_hours: answers.sitting || '',
    night_wakeups: answers.nightAwakenings || '',
    female_cycle: answers.femaleSpecific || '',
    libido: answers.libido || '',
    first_crash: answers.crash || '',
    weight_vs_ideal: answers.weightVsIdeal || '',
    digestion: answers.digestion || '',
    joint_pain: answers.jointPain || '',
    memory_focus: answers.cognition || '',
    hormonal_symptoms: answers.femaleSpecific || '', // Pour les femmes
    muscle_mass: answers.libido || '', // Pour les hommes (à adapter)
    recovery: answers.recovery || '',
    stress_level: answers.stress || '',
    skin_quality: answers.skin || '',
    living_environment: answers.environment || '',
    sun_exposure: answers.sun || '',
    nature_time: answers.nature || '',
    sleep_quality: answers.sleepQuality || '',
    bedtime: answers.bedtime || '',
    screens_before_bed: answers.screens || '',
    breakfast: answers.breakfast || '',
    hydration: answers.hydration || '',
    alcohol: answers.alcohol || '',
    activities: Array.isArray(answers.activities) ? answers.activities.join(', ') : (answers.activities || ''),
    sport_frequency: answers.frequency || '',
    supplements: answers.supplements || '',
    tracking: Array.isArray(answers.tracking) ? answers.tracking.join(', ') : (answers.tracking || ''),
    mental_load: '', // Question non présente dans votre quiz actuel
    professional_pressure: '', // Question non présente dans votre quiz actuel
    social_relations: answers.social || '',
    last_vacation: answers.vacations || '',
    projection_5years: answers.projection || '',
    biggest_fear: answers.fear || '',
    motivations: Array.isArray(answers.motivation) ? answers.motivation.join(', ') : (answers.motivation || ''),
